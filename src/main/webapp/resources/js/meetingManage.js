...
// 给删除按钮添加事件
    $("#recordBody").delegate(".btn-danger", "click", function () {

        var index = $(this).attr("id");
        var meetingID = json[index].MeetingID;
        var number = $(this).parents("tr").children()[0].innerHTML;
        //下面两个值用于搜索得到新数据
        var userName = $("#searchContent").val().trim();
        var meetingName = $("#select").val().trim();
        $.post("/Home/DelMeeting", { meetingID: meetingID, userName: userName, meetingName: meetingName }, function (data) {
            if (true) {
                json = data;
                currentPage = Math.ceil(number / num);//为了重定位到之前删除的地方
                totalData = json.length;
